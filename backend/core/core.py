...
class RecommendationEngine:
    ...
    async def _generate_ai_recommendation(self, ticker: str, data_sources: List[DataSourceSignal]) -> TradingRecommendation:
        """Generate AI-powered recommendation using OpenAI"""
        try:
            # Prepare context for AI
            context = self._prepare_ai_context(ticker, data_sources)
            ...
            response = await self.openai_client.chat.completions.create(
                ...
                messages=[
                    {
                        "role": "system",
                        "content": "You are an expert stock analyst. Provide a clear, actionable trading recommendation based on the provided data."
                    },
                    {
                        "role": "user",
                        "content": context
            ...
            recommendation = self._parse_ai_response(ticker, ai_response, data_sources)
            
            return recommendation
            
        except Exception as e:
            self.logger.error(f"AI recommendation generation failed for {ticker}: {e}")
            # Fall back to rule-based recommendation
            return await self._generate_rule_based_recommendation(ticker, data_sources)
    ...
    def _prepare_ai_context(self, ticker: str, data_sources: List[DataSourceSignal]) -> str:
        """Prepare context for AI analysis"""
        context = f"Please analyze {ticker} based on the following data:\n\n"
        
        for source in data_sources:
            context += f"{source.source} Analysis:\n"
            context += f"- Signal Strength: {source.signal_strength:.2f}\n"
            context += f"- Confidence: {source.confidence:.2f}\n"
            for point in source.key_points:
                context += f"- {point}\n"
            context += "\n"
        
        context += "Please provide:\n"
        context += "1. Clear recommendation (STRONG_BUY, BUY, HOLD, SELL, STRONG_SELL)\n"
        context += "2. Confidence level (0-1)\n"
        context += "3. Reasoning (2-3 sentences)\n"
        context += "4. Key risk factors\n"
        
        return context
    
    def _parse_ai_response(self, ticker: str, ai_response: str, data_sources: List[DataSourceSignal]) -> TradingRecommendation:
        ...
        risk_factors = ["AI-generated analysis"]
        
        # Try to extract recommendation type
        ai_upper = ai_response.upper()
        if "STRONG_BUY" in ai_upper:
            recommendation_type = RecommendationType.STRONG_BUY
            confidence = 0.8
        elif "STRONG_SELL" in ai_upper:
            recommendation_type = RecommendationType.STRONG_SELL
            confidence = 0.8
        elif "BUY" in ai_upper:
            recommendation_type = RecommendationType.BUY
            confidence = 0.7
        elif "SELL" in ai_upper:
            recommendation_type = RecommendationType.SELL
            confidence = 0.7
...
__all__ = [
    ...
    # Enums
    'SignalType', 'MarketRegime', 'DataSource', 'FeatureType',
