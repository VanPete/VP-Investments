...
# ============================================================================
# 3.0 CACHE-COMPATIBLE METHODS (Phase 1 Integration)
# ============================================================================

async def fetch_yfinance_technical_bundle(ticker: str, **kwargs) -> Dict[str, Any]:
    """
    Phase 1 compatible: Fetch technical data bundle for a ticker.
    
    Includes:
    - Price history (500 days OHLCV)
    - Corporate actions (dividends, splits)
    - Fast info (market cap, shares, beta)
    - Volume and volatility data
    
    Args:
        ticker: Stock ticker symbol
        period: History period (default: "500d")
        interval: Data interval (default: "1d")
        
    Returns:
        {
            "history": [...],  # OHLCV data as list of dicts
            "actions": {...},  # Dividends and splits
            "fast_info": {...}, # Market cap, shares, beta, float
    ...
    period = kwargs.get('period', '500d')
    interval = kwargs.get('interval', '1d')
    
    logger.info(f"🔍 Phase 1: Fetching yfinance technical bundle for {ticker}")
    
    try:
        # Fetch data (synchronous yfinance calls)
        ...
        if history_df.empty:
            ...
            current_price = float(history[-1]['Close']) if history else None
            volume = int(history[-1]['Volume']) if history else None
        
        # 2. Corporate actions
        actions = {}
        try:
            dividends = await asyncio.to_thread(lambda: yf_ticker.dividends)
            splits = await asyncio.to_thread(lambda: yf_ticker.splits)
            actions = {
                "dividends": dividends.to_dict() if hasattr(dividends, 'to_dict') else {},
                "splits": splits.to_dict() if hasattr(splits, 'to_dict') else {}
            }
        except Exception as e:
            logger.debug(f"Actions data unavailable for {ticker}: {e}")
            actions = {"dividends": {}, "splits": {}}
        
        # 3. Fast info (preferred over full info for speed)
        fast_info = {}
        try:
            fi = await asyncio.to_thread(lambda: yf_ticker.fast_info)
            if fi:
                fast_info = {
                    "market_cap": getattr(fi, 'market_cap', None),
                    "shares_outstanding": getattr(fi, 'shares', None),
                    "float_shares": getattr(fi, 'float_shares', None),
                    "last_price": getattr(fi, 'last_price', None),
                    "last_volume": getattr(fi, 'last_volume', None),
                    "currency": getattr(fi, 'currency', 'USD'),
                }
        except Exception as e:
            logger.debug(f"Fast info unavailable for {ticker}, trying full info: {e}")
            ...
            try:
                ...
                logger.warning(f"Both fast_info and info failed for {ticker}: {e2}")
        ...
        logger.info(f"✅ Phase 1: Technical bundle fetched for {ticker} ({len(history)} bars)")
        return bundle
        
    except Exception as e:
        logger.error(f"❌ Phase 1: Technical fetch failed for {ticker}: {e}")
        return {
            "history": [],
            "actions": {},
            "fast_info": {},
...
async def fetch_yfinance_fundamental_bundle(ticker: str, **kwargs) -> Dict[str, Any]:
    """
    Phase 1 compatible: Fetch fundamental data bundle for a ticker.
    ...
    logger.info(f"🔍 Phase 1: Fetching yfinance fundamental bundle for {ticker}")
    
    def df_to_records_safe(df):
        """Safely convert DataFrame to records, handling NaNs"""
        if df is None or (hasattr(df, 'empty') and df.empty):
            return []
        try:
            records = df.reset_index().to_dict('records')
            # Convert Timestamp objects and handle NaNs
            for row in records:
    ...
    try:
        ...
        financials_annual = await asyncio.to_thread(lambda: yf_ticker.financials)
        financials_quarterly = await asyncio.to_thread(lambda: yf_ticker.quarterly_financials)
        balance_sheet_annual = await asyncio.to_thread(lambda: yf_ticker.balance_sheet)
        balance_sheet_quarterly = await asyncio.to_thread(lambda: yf_ticker.quarterly_balance_sheet)
        cashflow_annual = await asyncio.to_thread(lambda: yf_ticker.cashflow)
        cashflow_quarterly = await asyncio.to_thread(lambda: yf_ticker.quarterly_cashflow)
        
        # Earnings data
        earnings_dates = []
        try:
            earnings_df = await asyncio.to_thread(lambda: yf_ticker.earnings_dates)
            earnings_dates = df_to_records_safe(earnings_df)
        except Exception as e:
            logger.debug(f"Earnings dates unavailable for {ticker}: {e}")
        
        bundle = {
            "financials_annual": df_to_records_safe(financials_annual),
            "financials_quarterly": df_to_records_safe(financials_quarterly),
            "balance_sheet_annual": df_to_records_safe(balance_sheet_annual),
            "balance_sheet_quarterly": df_to_records_safe(balance_sheet_quarterly),
            "cashflow_annual": df_to_records_safe(cashflow_annual),
            "cashflow_quarterly": df_to_records_safe(cashflow_quarterly),
            "earnings_dates": earnings_dates,
            "metadata": {
                "ticker": ticker,
                "fetched_at": datetime.now(timezone.utc).isoformat(),
                "statements_available": {
                    "income": len(df_to_records_safe(financials_annual)) > 0,
                    "balance_sheet": len(df_to_records_safe(balance_sheet_annual)) > 0,
                    "cashflow": len(df_to_records_safe(cashflow_annual)) > 0,
                }
            }
        }
        
        logger.info(f"✅ Phase 1: Fundamental bundle fetched for {ticker}")
        return bundle
        
    except Exception as e:
        logger.error(f"❌ Phase 1: Fundamental fetch failed for {ticker}: {e}")
        return {
            "financials_annual": [],
            "financials_quarterly": [],
            "balance_sheet_annual": [],
...
async def fetch_yfinance_risk_bundle(ticker: str, **kwargs) -> Dict[str, Any]:
    """
    Phase 1 compatible: Fetch risk/stability data bundle for a ticker.
    
    Includes:
    - Beta, volatility
    - Short interest data
    - Options data (IV, put/call ratios)
    - Liquidity metrics
    ...
    logger.info(f"🔍 Phase 1: Fetching yfinance risk bundle for {ticker}")
    ...
    try:
        ...
        short_interest = {
            ...
            "short_percent_outstanding": info.get('shortPercentageOfSharesOutstanding'),
        ...
        options_data = {}
        try:
            options_dates = await asyncio.to_thread(lambda: yf_ticker.options)
            if options_dates and len(options_dates) > 0:
                # Get nearest expiration
                nearest_exp = options_dates[0]
                opt_chain = await asyncio.to_thread(yf_ticker.option_chain, nearest_exp)
                ...
                if opt_chain:
                    ...
                    options_data = {
                        ...
                        "implied_volatility_avg": float(calls['impliedVolatility'].mean()) if not calls.empty else None,
                        "put_call_volume_ratio": float(puts['volume'].sum() / calls['volume'].sum()) if not calls.empty and not puts.empty else None,
                        "put_call_oi_ratio": float(puts['openInterest'].sum() / calls['openInterest'].sum()) if not calls.empty and not puts.empty else None,
                        "total_volume": int(calls['volume'].sum() + puts['volume'].sum()) if not calls.empty and not puts.empty else None,
                        "total_open_interest": int(calls['openInterest'].sum() + puts['openInterest'].sum()) if not calls.empty and not puts.empty else None,
                    }
        except Exception as e:
        ...
        bundle = {
            ...
            "metadata": {
                "ticker": ticker,
                "fetched_at": datetime.now(timezone.utc).isoformat(),
                "options_available": len(options_data) > 0,
            }
        }
        
        logger.info(f"✅ Phase 1: Risk bundle fetched for {ticker}")
        return bundle
        
    except Exception as e:
        logger.error(f"❌ Phase 1: Risk fetch failed for {ticker}: {e}")
        return {
            "beta": None,
            "short_interest": {},
            "options": {},
...
async def fetch_yfinance_institutional_bundle(ticker: str, **kwargs) -> Dict[str, Any]:
    """
    Phase 1 compatible: Fetch institutional/smart money data bundle.
    ...
    logger.info(f"🔍 Phase 1: Fetching yfinance institutional bundle for {ticker}")
    ...
    try:
        ...
        institutional_holders = []
        try:
            inst_df = await asyncio.to_thread(lambda: yf_ticker.institutional_holders)
            institutional_holders = df_to_records_safe(inst_df)
        except Exception as e:
            logger.debug(f"Institutional holders unavailable for {ticker}: {e}")
        
        # Insider transactions
        insider_transactions = []
        try:
            insider_df = await asyncio.to_thread(lambda: yf_ticker.insider_transactions)
            insider_transactions = df_to_records_safe(insider_df)
        except Exception as e:
            logger.debug(f"Insider transactions unavailable for {ticker}: {e}")
        
        # Major holders
        major_holders = {}
        try:
            major_df = await asyncio.to_thread(lambda: yf_ticker.major_holders)
            if major_df is not None and not major_df.empty:
                major_holders = major_df.to_dict()
        except Exception as e:
            logger.debug(f"Major holders unavailable for {ticker}: {e}")
        
        # Analyst recommendations
        recommendations = []
        try:
            rec_df = await asyncio.to_thread(lambda: yf_ticker.recommendations)
            recommendations = df_to_records_safe(rec_df)
        except Exception as e:
            logger.debug(f"Recommendations unavailable for {ticker}: {e}")
        ...
        bundle = {
            ...
            "metadata": {
                ...
                "insider_transactions_count": len(insider_transactions),
                "analyst_recommendations_count": len(recommendations),
            }
        }
        
        logger.info(f"✅ Phase 1: Institutional bundle fetched for {ticker}")
        return bundle
        
    except Exception as e:
        logger.error(f"❌ Phase 1: Institutional fetch failed for {ticker}: {e}")
        return {
            "institutional_holders": [],
            "insider_transactions": [],
            "major_holders": {},
...
async def fetch_yfinance_complete_bundle(ticker: str, **kwargs) -> Dict[str, Any]:
    """
    Phase 1 compatible: Fetch ALL yfinance data in one call.
    
    This is the MASTER fetch function that combines all groups.
    Use this for eager bundling in Phase 1.
    ...
    logger.info(f"🔍 Phase 1: Fetching COMPLETE yfinance bundle for {ticker}")
    
    # Fetch all bundles in parallel
    results = await asyncio.gather(
        fetch_yfinance_technical_bundle(ticker, **kwargs),
        fetch_yfinance_fundamental_bundle(ticker, **kwargs),
        fetch_yfinance_risk_bundle(ticker, **kwargs),
        fetch_yfinance_institutional_bundle(ticker, **kwargs),
        return_exceptions=True
    )
    
    technical, fundamental, risk, institutional = results
    
    # Handle any exceptions
    if isinstance(technical, Exception):
        logger.error(f"Technical bundle error: {technical}")
        technical = {"metadata": {"error": str(technical)}}
    if isinstance(fundamental, Exception):
        logger.error(f"Fundamental bundle error: {fundamental}")
        fundamental = {"metadata": {"error": str(fundamental)}}
    if isinstance(risk, Exception):
        logger.error(f"Risk bundle error: {risk}")
        risk = {"metadata": {"error": str(risk)}}
    if isinstance(institutional, Exception):
        logger.error(f"Institutional bundle error: {institutional}")
        institutional = {"metadata": {"error": str(institutional)}}
    ...
    logger.info(f"✅ Phase 1: COMPLETE bundle fetched for {ticker}")
    return complete_bundle


# ============================================================================
# FACTORY FUNCTIONS FOR 3.0 PIPELINE
# ============================================================================

def get_yfinance_fetcher():
    """
    Factory function to get YahooFinanceIntegrator instance.
    Used by Phase 1 cache layer.
    """
    return YahooFinanceIntegrator()


# Singleton instances for backward compatibility
technical_calculator = TechnicalIndicatorCalculator()
financial_calculator = ImprovedFinancialCalculator()
