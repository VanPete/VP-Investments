...
class RiskScoreCalculator:
    ...
    # NOTE: AI risk narrative generation moved to backend/integrations/ai.py (Phase 6)
    # Use AICommentaryGenerator for AI-powered risk narratives in pipeline Phase 6
...
class SignalScorer:
    """
    ⚠️ DEPRECATED: This class is being replaced by v3.1 modular pipeline:
    - Phase 2: Calculate factors (145 factors across 6 groups)
    - Phase 3: Normalize factors (robust z-score)
    - Phase 4: Score & assemble (weighted scoring)
    
    Use test_integrated_v3_1.py to see the new architecture in action.
    This class remains for backward compatibility only.
    
    OLD Multi-factor signal scoring engine with configurable profiles
    ...
    def __init__(self, profile: str = "ml_optimized", db_manager=None):
        import warnings
        warnings.warn(
            "SignalScorer is deprecated. Use v3.1 modular pipeline: "
            "Phase2Calculator -> Phase3Normalizer -> Phase4ScoreAssembler",
            DeprecationWarning,
            stacklevel=2
        )
    ...
    # All data must be pre-fetched and passed in ticker_data parameter
    # No mid-pipeline API calls allowed in 3.0 architecture
    
    async def score_ticker(self, ticker_data: Dict) -> SignalResult:
        """
        Score a single ticker using Phase 3-4 comprehensive 6-group scoring system.
        
        3.0 Architecture (Phases 3-4):
        - Phase 1: All data pre-fetched and cached (yfinance.py + cache.py)
        - Phase 2: Data parsed and normalized (calculator.py)
        - Phase 3: Score by group (THIS METHOD - uses pre-fetched data)
        - Phase 4: Assemble final signal
        
        Expected in ticker_data:
        - Basic fields: ticker, current_price, volume, market_cap
        - Technical fields: rsi, macd, sma_50, sma_200, etc.
        - Fundamental fields: pe_ratio, peg_ratio, revenue_growth, margins, etc.
        - News fields: news_score, news_sentiment, news_count
        - Social fields: reddit_mentions, reddit_sentiment, reddit_score
        - Risk fields: beta, volatility, sharpe_ratio
        - Z-scores: technical_z, fundamental_z, news_z, social_z, etc.
        - Institutional fields: institutional_ownership, insider_activity
        ...
        try:
            ...
            # 3.0: All data must be pre-fetched in Phase 1
            # No API calls allowed here - only scoring logic
            
            # Calculate 6 component scores (Phase 3)
            component_scores = {
            ...
            # Phase 3: Advanced trade classification (uses pre-fetched data)
            trade_tags, classification_details = self.trade_classifier.classify_trade_type(
                ticker, ticker_data, component_scores, self.db_manager
            )
            
            # Phase 3: Advanced risk scoring (uses pre-fetched data)
            risk_score, risk_level, risk_factors = self.risk_calc.calculate_risk_score(
                ticker, ticker_data, classification_details.get('theme')
            )
            
            # Phase 3: Generate template-based risk narrative (AI moved to Phase 6)
            ...
            # Phase 3: Dynamic weight adjustment by trade type
            ...
            return SignalResult(
                ...
                risk_assessment=risk_assessment,  # Phase 3: Template-based narrative (AI in Phase 6)
                theme=classification_details.get('theme'),
                event_flags=classification_details.get('event_flags'),
                # Phase 2: All z-scores calculated in calculator.py and passed via ticker_data
                technical_z=ticker_data.get('technical_z'),
                fundamental_z=ticker_data.get('fundamental_z'),
                news_z=ticker_data.get('news_z'),
                social_z=ticker_data.get('social_z'),
                trend_strength_z=ticker_data.get('trend_strength_z'),
                valuation_z=ticker_data.get('valuation_z'),
                ma_slope_50=ticker_data.get('ma_slope_50'),
                ma_slope_200=ticker_data.get('ma_slope_200'),
                volume_trend_z=ticker_data.get('volume_trend_z'),
                price_z_20day=ticker_data.get('price_z_20day'),
                atr_pct=ticker_data.get('atr_pct'),
                float_pct=ticker_data.get('float_pct'),
    ...
    def _calculate_reddit_score(self, mention_count: Optional[int] = None, avg_sentiment: Optional[float] = None, 
                               avg_score: Optional[float] = None, data: Optional[Dict] = None) -> float:
        """
    ...
    def _calculate_news_score(self, news_data: Optional[Dict[str, Any]] = None, data: Optional[Dict] = None) -> float:
        """
    ...
    def _calculate_score_components(self, signal: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            # Technical factor contributions (simplified - Phase 7 handles these internally)
            technical_factors = {
                'rsi_factor': 0.5,  # Neutral - Phase 7 handles technical scoring
                'macd_factor': 0.5,
                'volume_factor': 0.5,
                'momentum_factor': 0.5,
            ...
            # Generate score explanation (simplified)
            signal['score_explanation'] = f"Score {signal_score:.3f} based on 6-group Phase 7 scoring system"
            
            # Set scoring metadata
            signal['scoring_version'] = '1.0'
            
            # Use Phase 7 confidence if available, otherwise use default
            ...
            if 'phase7_confidence' in signal:
                ...
                # Default confidence based on signal score
                default_confidence = min(max(signal_score, 0.0), 1.0)
                signal['prediction_confidence'] = default_confidence
                # Also set confidence for consistency
                if 'confidence' not in signal:
                    signal['confidence'] = default_confidence
    ...
    def _calculate_fundamental_score(self, data: Dict[str, Any]) -> float:
        ...
        result = _calculate_fundamental_score_standalone(data)
    ...
    def _calculate_social_alternative_score(self, data: Dict[str, Any]) -> float:
        ...
        result = _calculate_social_alternative_score_standalone(data)
        return result if result is not None else 0.0
...
def _calculate_fundamental_score_standalone(financial_data: Dict[str, Any]) -> Optional[float]:
    """
...
def _calculate_social_alternative_score_standalone(reddit_data: Dict[str, Any]) -> Optional[float]:
    """
